...
let inlineCompletionTimeout: NodeJS.Timeout | undefined;
let lastCompletionPosition: vscode.Position | undefined;
        ...
        ],
        {
            async provideInlineCompletionItems(
                ...
                const textAfterCursor = document.getText(
                    new vscode.Range(position, new vscode.Position(position.line + 10, 0))
                );
                ...
                try {
                    ...
                    const response = await fetch(`${API_URL}/inline-complete`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
...
async function checkApiHealth(statusBarItem: vscode.StatusBarItem): Promise<void> {
    try {
